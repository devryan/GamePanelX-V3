...
class Network
{
        ...
        */
        ...
        else
        {
            ...
            if(!preg_match('/^(AutoInstall|CheckCreateServerStatus|ChangePassword|CheckInstall|CheckTemplates|CreateUser|CreateTemplate|DeleteUser|DeleteTemplate|UsernameChange|SteamCMDInstall|SteamInstall)\ /', $cmd))
            {
                ...
                // Get sso user/pass
                $result_sso = $GLOBALS['mysqli']->query("SELECT
                                            ...
                                            LEFT JOIN servers AS s ON 
                                              u.id = s.userid 
                                            WHERE 
                                              s.id = '$gamesrv_id'") or die('Failed to query for sso info: '.$GLOBALS['mysqli']->error);
    ...
    public function create($ip,$is_local,$os,$datacenter,$location,$login_user,$login_pass,$login_port)
    {
        ...
        $result_ck  = $GLOBALS['mysqli']->query("SELECT id FROM network WHERE ip = '$ip'") or die('Failed to check network');
        ...
        // Insert
        $GLOBALS['mysqli']->query("INSERT INTO network (ip,token,is_local,os,datacenter,location,login_user,login_pass,login_port) VALUES('$ip','$remote_token','$is_local','$os','$datacenter','$location',AES_ENCRYPT('$login_user', '$enc_key'),AES_ENCRYPT('$login_pass', '$enc_key'),AES_ENCRYPT('$login_port', '$enc_key'))") or die('Failed to insert the network server: '.$GLOBALS['mysqli']->error);
        $this_netid = $GLOBALS['mysqli']->insert_id;
        ...
        else
        {
            ...
            if(preg_match('/^\//', $net_homedir))
            {
                ...
                $GLOBALS['mysqli']->query("UPDATE network SET homedir = '$net_homedir' WHERE id = '$this_netid'") or die('Failed to update homedir: '.$GLOBALS['mysqli']->error);
                
                ########################################################
                
                // Get list of user accounts to be created if needed on the network server
                $result_users   = $GLOBALS['mysqli']->query("SELECT username FROM users WHERE deleted = '0' ORDER BY username ASC");
                ...
                else
                {
                    ...
                    $GLOBALS['mysqli']->query("DELETE FROM network WHERE id = '$this_netid'") or die('Failed to delete network server: '.$GLOBALS['mysqli']->error);
    ...
    public function delete($netid)
    {
        ...
        $result_ip  = $GLOBALS['mysqli']->query("SELECT id FROM servers WHERE netid = '$netid' LIMIT 1") or die('Failed to get IP!');
        $row_ip     = $result_ip->fetch_row();
        if($row_ip[0]) return $lang['srv_using_net'];
        
        // Delete templates (we warned them!)
        $GLOBALS['mysqli']->query("DELETE FROM templates WHERE netid = '$netid'") or die('Failed to delete the network server');
        
        // Delete ID and all with this as a parent ID
        $GLOBALS['mysqli']->query("DELETE FROM network WHERE id = '$netid' OR parentid = '$netid'") or die('Failed to delete the network server');
