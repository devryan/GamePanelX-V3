...
class Templates
{
    ...
    public function create($netid,$gameid,$file_path,$description,$is_def)
    {
        ...
        if(empty($file_path))
        {
            ...
            $result_stm = @mysqli_query("SELECT steam,steam_name,install_mirrors,install_cmd FROM default_games WHERE id = '$gameid' LIMIT 1");
	...
	################################################################
        ...
        if($is_def) @mysqli_query("UPDATE templates SET is_default = '0' WHERE netid = '$netid' AND cfgid = '$gameid'");
        
        // Insert
        @mysqli_query("INSERT INTO templates (netid,cfgid,date_created,is_default,status,token,description,file_path) VALUES('$netid','$gameid',NOW(),'$is_def','$tpl_status','$remote_token','$description','$file_path')") or die('Failed to insert template');
        $tpl_id = mysqli_insert_id();
        ...
        if($is_steam)
        {
              ...
              else
              {
                  ...
                  @mysqli_query("DELETE FROM templates WHERE id = '$tpl_id'") or die('Failed to delete the template from the database');
    ...
    public function delete($tplid)
    {
        ...
        // Get netid
        $result_nid = @mysqli_query("SELECT netid FROM templates WHERE id = '$tplid' LIMIT 1");
        $row_nid    = mysqli_fetch_row($result_nid);
        $netid      = $row_nid[0];
        
        // Delete from DB
        @mysqli_query("DELETE FROM templates WHERE id = '$tplid'") or die('Failed to delete the template row!');
