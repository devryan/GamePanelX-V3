    ...
    public function update($userid,$username,$password,$email,$first_name,$last_name,$language,$theme)
    {
        ...
        if(isset($_SESSION['gpx_admin']))
        {
			...
			if($cur_username != $username || !empty($password))
			{
				...
				@mysql_query("UPDATE users SET last_updated = NOW(),sso_user = AES_ENCRYPT('$sso_user', '$enc_key'),sso_pass = AES_ENCRYPT('$sso_pass', '$enc_key'),language = '$language',username = '$username',email_address = '$email',first_name = '$first_name',last_name = '$last_name'$sql_pass WHERE id = '$userid'") or die('Failed to update user');
			}
        }
