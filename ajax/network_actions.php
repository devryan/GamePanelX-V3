...
{
    @mysqli_query($connection, "DELETE FROM network WHERE id = '$url_id'") or die('Failed to delete the IP Address');
...
{
    // Get original IP
    $result_ip  = @mysqli_query($connection, "SELECT ip FROM network WHERE id = '$url_id' LIMIT 1") or die('Failed to get IP!');
...
{
    // Check existing
    $result_ip  = @mysqli_query($connection, "SELECT id FROM network WHERE ip = '$url_ip' LIMIT 1") or die('Failed to get IP!');
    $row_ip     = mysqli_fetch_row($result_ip);
    ...
    @mysqli_query($connection, "INSERT INTO network (parentid,ip) VALUES('$url_id','$url_ip')") or die('Failed to add the IP Address: '.mysqli_error($connection));
