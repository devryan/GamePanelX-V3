...
elseif($url_do == 'settings_save')
{
	...
	if(GPXDEBUG) echo "Saving description: $url_descr, for ID: $url_id<br>";
        ...
        @mysqli_query("UPDATE servers SET 
                      ...
                      WHERE id = '$url_id'") or die('Failed to update admin server settings: '.mysqli_error());
    }
...
elseif($url_do == 'startup_save')
{
    ...
    if(isset($_SESSION['gpx_admin']))
    {
        if($startup_type == 'smp')
        {
            @mysqli_query("UPDATE servers SET startup = '0' WHERE id = '$url_id'") or die('Failed to update startup type');
            echo 'success';
            exit;
        }
        ...
        {
            @mysqli_query("UPDATE servers SET startup = '1' WHERE id = '$url_id'") or die('Failed to update startup type');
	...
	// Only update if changed
        if(strlen($update_item_query) > 72) @mysqli_query($update_item_query) or die('Failed to update items: '.mysqli_error());
        if(strlen($update_usred_query) > 72) @mysqli_query($update_usred_query) or die('Failed to update user editable: '.mysqli_error());
        if($sort_order) @mysqli_query($update_sort_query) or die('Failed to update sorting order: '.mysqli_error());
    }
    
    // Run updates only if new stuff
    if(strlen($update_val_query) > 74) @mysqli_query($update_val_query) or die('Failed to update values: '.mysqli_error());
...
elseif($url_do == 'startup_del_item')
{
    ...
    @mysqli_query("DELETE FROM servers_startup WHERE id = '$url_id' AND srvid = '$server_id'") or die('Failed to delete the startup item');
...
elseif($url_do == 'create_getport')
{
    ...
    $result_port  = @mysqli_query("SELECT 
    				    d.port 
...
elseif($url_do == 'multi_query')
{
    ...
    $result_srv = @mysqli_query("SELECT 
                                ...
                                ORDER BY 
                                  s.id DESC,
                                  n.ip ASC 
                                LIMIT 30") or die($lang['err_query'].' ('.mysqli_error().')');
    ...
    $result_sv  = @mysqli_query("SELECT
				...
				GROUP BY 
				  t.id
                                ORDER BY 
                                  d.name ASC,
                                  t.is_default DESC") or die('<option value="">Failed to query for games: '.mysqli_error().'</option>');
