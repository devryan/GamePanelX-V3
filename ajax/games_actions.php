...
elseif($url_do == 'save')
{
    ...
    @mysqli_query("UPDATE default_games 
                    SET 
                   ...
                   WHERE 
                      id = '$url_id'") or die('Failed to update game: '.mysqli_error());
...
elseif($url_do == 'add')
{
    ...
    @mysqli_query("INSERT INTO default_games (port,maxplayers,steam,type,gameq_name,name,intname,working_dir,pid_file,map,hostname,config_file,steam_name,description,update_cmd,simplecmd) 
                  ...
                  '$game_config_file','$game_steam_name','$game_descr','$game_updatecmd','$game_simplecmd')") or die('Failed to add the game: '.mysqli_error());
...
elseif($url_do == 'startup_save')
{
    ...
    if($startup_type == 'smp')
    {
        @mysqli_query("UPDATE default_games SET startup = '0' WHERE id = '$url_id'") or die('Failed to update startup type');
        echo 'success';
        exit;
    }
    ...
    {
        @mysqli_query("UPDATE default_games SET startup = '1' WHERE id = '$url_id'") or die('Failed to update startup type');
    }
    ...
    if(!preg_match('/VALUES$/', $add_query))
    {
        ...
        @mysqli_query($add_query) or die('Failed to add items: '.mysqli_error());
    }
    
    // Run updates
    if($hascur)
    {
        @mysqli_query($update_item_query) or die('Failed to update items: '.mysqli_error());
        @mysqli_query($update_val_query) or die('Failed to update values: '.mysqli_error());
        @mysqli_query($update_usred_query) or die('Failed to update user editable: '.mysqli_error());
        if($sort_order) @mysqli_query($update_sort_query) or die('Failed to update order: '.mysqli_error());
    }
    
    // Update simplecmd with most recent order
    $simplecmd  = '';
    $result_smp = @mysqli_query("SELECT cmd_item,cmd_value FROM default_startup WHERE defid = '$url_id' ORDER BY sort_order ASC") or die('Failed to get item/vals!');
    ...
    @mysqli_query("UPDATE default_games SET simplecmd = '$simplecmd' WHERE id = '$url_id'") or die('Failed to update simplecmd!');
...
elseif($url_do == 'startup_del_item')
{
    ...
    @mysqli_query("DELETE FROM default_startup WHERE id = '$url_id' AND defid = '$server_id'") or die('Failed to delete the startup item');
...
elseif($url_do == 'delete')
{
    ...
    $result_chk = @mysqli_query("SELECT id FROM servers WHERE defid = '$url_id' LIMIT 1");
    $row_chk    = mysqli_fetch_row($result_chk);
    if($row_chk[0]) die('There are servers using this game!  Delete them first and try again.');
    
    @mysqli_query("DELETE FROM default_games WHERE id = '$url_id'") or die('Failed to delete the game setup');
...
{
    // Get game info
    $result_info  = @mysqli_query("SELECT * FROM default_games WHERE id = '$url_id' ORDER BY id DESC LIMIT 1") or die('Failed to query for game info!');
    $game_arr     = array();
    ...
    $result_strt  = @mysqli_query("SELECT * FROM default_startup WHERE defid = '$url_id' ORDER BY sort_order ASC") or die('Failed query for game setup items!');
    $strt_arr     = array();
