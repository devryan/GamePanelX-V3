...
$cld_date_created   = mysqli_real_escape_string($cloud_arr[0]['date_created']);
$cld_last_updated   = mysqli_real_escape_string($cloud_arr[0]['last_updated']);
$cld_is_steam       = mysqli_real_escape_string($cloud_arr[0]['steam']);
$cld_steam_name     = mysqli_real_escape_string($cloud_arr[0]['steam_name']);
$cld_name           = mysqli_real_escape_string($cloud_arr[0]['name']);
$cld_description    = mysqli_real_escape_string($cloud_arr[0]['description']);
$cld_icon           = mysqli_real_escape_string($cloud_arr[0]['icon']);
$cld_port           = mysqli_real_escape_string($cloud_arr[0]['port']);
$cld_gameq          = mysqli_real_escape_string($cloud_arr[0]['gameq_name']);
$cld_intname        = mysqli_real_escape_string($cloud_arr[0]['intname']);
$cld_working_dir    = mysqli_real_escape_string($cloud_arr[0]['working_dir']);
$cld_pid_file       = mysqli_real_escape_string($cloud_arr[0]['pid_file']);
$cld_simplecmd      = mysqli_real_escape_string($cloud_arr[0]['simplecmd']);
$cld_update_cmd     = mysqli_real_escape_string($cloud_arr[0]['update_cmd']);
$cld_banned_chars   = mysqli_real_escape_string($cloud_arr[0]['banned_chars']);
$cld_maxpl          = mysqli_real_escape_string($cloud_arr[0]['maxplayers']);
$cld_startup        = mysqli_real_escape_string($cloud_arr[0]['startup']);
$cld_type           = mysqli_real_escape_string($cloud_arr[0]['type']);
$cld_cfg_sep        = mysqli_real_escape_string($cloud_arr[0]['cfg_separator']);
$cld_cfg_ip         = mysqli_real_escape_string($cloud_arr[0]['cfg_ip']);
$cld_cfg_port       = mysqli_real_escape_string($cloud_arr[0]['cfg_port']);
$cld_cfg_maxpl      = mysqli_real_escape_string($cloud_arr[0]['cfg_maxplayers']);
$cld_cfg_map        = mysqli_real_escape_string($cloud_arr[0]['cfg_map']);
$cld_cfg_hostname   = mysqli_real_escape_string($cloud_arr[0]['cfg_hostname']);
$cld_cfg_rcon       = mysqli_real_escape_string($cloud_arr[0]['cfg_rcon']);
$cld_cfg_passw      = mysqli_real_escape_string($cloud_arr[0]['cfg_password']);
$cld_map            = mysqli_real_escape_string($cloud_arr[0]['map']);
$cld_hostname       = mysqli_real_escape_string($cloud_arr[0]['hostname']);
$cld_config_file    = mysqli_real_escape_string($cloud_arr[0]['config_file']);
$cld_inst_mirr      = mysqli_real_escape_string($cloud_arr[0]['install_mirrors']);
$cld_inst_cmd       = mysqli_real_escape_string($cloud_arr[0]['install_cmd']);
...
// Get default ID
$result_id  = @mysqli_query("SELECT id FROM default_games WHERE cloudid = '$url_id' ORDER BY id LIMIT 1");
$row_id     = mysqli_fetch_row($result_id);
$def_id     = $row_id[0];

// Delete any existing default rows or startup items for this game
@mysqli_query("DELETE FROM default_games WHERE cloudid = '$url_id'");
@mysqli_query("DELETE FROM default_startup WHERE defid = '$def_id'");
...
if(!empty($cloud_arr[1]))
{
    ...
    // Run the insert
    @mysqli_query($startup_sql) or die('Failed to insert startup items');
}
...
@mysqli_query("UPDATE servers SET defid = '$this_defid' WHERE defid = '$def_id'") or die('Failed to update servers');
@mysqli_query("UPDATE templates SET cfgid = '$this_defid' WHERE cfgid = '$def_id'") or die('Failed to update templates');
