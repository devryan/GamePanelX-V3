...
// Setup db
$GLOBALS['mysqli'] = new mysqli($settings['db_host'], $settings['db_username'], $settings['db_password']) or die('Failed to connect to the database!  Check your settings and try again.');
$GLOBALS['mysqli']->select_db($settings['db_name']) or die('Failed to select the database!  Check your settings and try again.');

// Get current version
$result_cfg   = $GLOBALS['mysqli']->query("SELECT config_value FROM configuration WHERE config_setting = 'version' ORDER BY last_updated DESC LIMIT 1");
...
function update_gpxver($this_ver)
{
    ...
    $GLOBALS['mysqli']->query("UPDATE configuration SET config_value = '$new_version' WHERE config_setting = 'version'") or die('Failed to update version: '.$GLOBALS['mysqli']->error);
...
{
    $GLOBALS['mysqli']->query("INSERT INTO configuration (config_setting,config_value) VALUES('version','3.0.3')");
    $cur_version = '3.0.3';
}
...
if(version_compare($cur_version, '3.0.4') == -1)
{
    ...
    $GLOBALS['mysqli']->query("ALTER TABLE default_games ADD `banned_chars` VARCHAR(64) NOT NULL AFTER pid_file") or die('Failed to add banned_chars: '.$GLOBALS['mysqli']->error);
    
    // Add default banned chars to cs series games
    $GLOBALS['mysqli']->query("UPDATE default_games SET banned_chars = '+- ' WHERE intname IN('cs_16','cs_cz','cs_s','cs_go')") or die('Failed to update banned chars: '.$GLOBALS['mysqli']->error);
...
if(version_compare($cur_version, '3.0.6') == -1)
{
    ...
    // Minecraft updates
    $GLOBALS['mysqli']->query("UPDATE default_games SET gameq_name = 'minecraft',cloudid = '6',simplecmd = 'java -Xincgc -Xmx1000M -jar craftbukkit.jar nogui' WHERE intname = 'mcraft'") or die('Failed to update Minecraft support: '.$GLOBALS['mysqli']->error);
    
    // Counter-Strike updates
    $GLOBALS['mysqli']->query("UPDATE default_games SET update_cmd = './steam -command update -game cstrike -dir .' WHERE intname = 'cs_16'") or die('Failed to update CS 1.6 support: '.$GLOBALS['mysqli']->error);
    $GLOBALS['mysqli']->query("UPDATE default_games SET update_cmd = './steam -command update -game czero -dir .' WHERE intname = 'cs_cz'") or die('Failed to update CS CZ support: '.$GLOBALS['mysqli']->error);
    $GLOBALS['mysqli']->query("UPDATE default_games SET update_cmd = './steam -command update -game \'Counter-Strike Source\' -dir .' WHERE intname = 'cs_s'") or die('Failed to update CS S support: '.$GLOBALS['mysqli']->error);
    $GLOBALS['mysqli']->query("UPDATE default_games SET cloudid = '7' WHERE intname = 'cs_go'") or die('Failed to update CS GO support: '.$GLOBALS['mysqli']->error);
    
    // Add language support
    $GLOBALS['mysqli']->query("ALTER TABLE admins ADD `language` VARCHAR(64) NOT NULL DEFAULT 'english' AFTER `password`") or die('Failed to add admin language: '.$GLOBALS['mysqli']->error);
    $GLOBALS['mysqli']->query("ALTER TABLE users ADD `language` VARCHAR(64) NOT NULL DEFAULT 'english' AFTER `last_updated`") or die('Failed to add user language: '.$GLOBALS['mysqli']->error);
...
if(version_compare($cur_version, '3.0.8') == -1)
{
    ...
    // Add theme support
    $GLOBALS['mysqli']->query("ALTER TABLE admins ADD `theme` VARCHAR(64) NOT NULL DEFAULT 'default' AFTER `password`") or die('Failed to add admin theme: '.$GLOBALS['mysqli']->error);
    $GLOBALS['mysqli']->query("ALTER TABLE users ADD `theme` VARCHAR(64) NOT NULL DEFAULT 'default' AFTER `last_updated`") or die('Failed to add user theme: '.$GLOBALS['mysqli']->error);
    
    // Add new columns to `default_games`
    $GLOBALS['mysqli']->query("ALTER TABLE `default_games` 
                    ...
                    ADD `config_file` VARCHAR(255) NOT NULL AFTER `cfg_password`") or die('Failed to add default_games columns: '.$GLOBALS['mysqli']->error);
    
    // Add new columns to `servers`
    $GLOBALS['mysqli']->query("ALTER TABLE `servers` 
                    ADD `maxplayers` SMALLINT(4) UNSIGNED NOT NULL AFTER `port`,
                    ...
                    ADD `sv_password` VARCHAR(255) NOT NULL AFTER `hostname`") or die('Failed to add default_games columns: '.$GLOBALS['mysqli']->error);
    ...
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET `config_file` = CASE `intname` 
                  ...
                  ELSE `config_file` END") or die('Failed to update default games: '.$GLOBALS['mysqli']->error);
    ...
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET `cfg_separator` = CASE `intname` 
                      ...
                      WHEN 'bf2' THEN ' '
                      WHEN 'vent' THEN '='
                  ELSE `cfg_separator` END") or die('Failed to update separators: '.$GLOBALS['mysqli']->error);
    ...
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET `cfg_ip` = CASE `intname` 
                  ...
                  ELSE `cfg_ip` END") or die('Failed to update ips: '.$GLOBALS['mysqli']->error);
    ...
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET `cfg_port` = CASE `intname` 
                  ...
                  ELSE `cfg_port` END") or die('Failed to update ports: '.$GLOBALS['mysqli']->error);
    ...
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET `cfg_maxplayers` = CASE `intname` 
                  ...
                  ELSE `cfg_maxplayers` END") or die('Failed to update maxplayers: '.$GLOBALS['mysqli']->error);
    ...
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET `cfg_map` = CASE `intname` 
                  ...
                  ELSE `cfg_map` END") or die('Failed to update map: '.$GLOBALS['mysqli']->error);
    ...
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET `cfg_hostname` = CASE `intname` 
                      ...
                      WHEN 'vent' THEN 'Name'
                  ELSE `cfg_hostname` END") or die('Failed to update hostname: '.$GLOBALS['mysqli']->error);
    ...
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET `cfg_rcon` = CASE `intname` 
                  ...
                  ELSE `cfg_rcon` END") or die('Failed to update rcon: '.$GLOBALS['mysqli']->error);
    
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET `cfg_password` = CASE `intname` 
                      ...
                      WHEN 'gta_samp' THEN 'rcon_password'
                      WHEN 'vent' THEN 'AdminPassword'
                  ELSE `cfg_password` END") or die('Failed to update rcon: '.$GLOBALS['mysqli']->error);
    
    ########
    
    // Update install config for minecraft
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET port = '25565',install_mirrors = 'http://dl.bukkit.org/latest-rb/craftbukkit.jar',install_cmd = 'mv craftbukkit* craftbukkit.jar' WHERE intname = 'mcraft'") or die('Failed to update minecraft: '.$GLOBALS['mysqli']->error);
    
    // Update CS:GO steam name from "csgo" to "740" to use steamcmd app ID
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET steam = '2',steam_name = '740' WHERE intname = 'cs_go'") or die('Failed to update csgo: '.$GLOBALS['mysqli']->error);
    
    // Update `startup` to 0 for non-startup games
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET `startup` = '0' WHERE intname IN ('vent','bf2','mcraft')") or die('Failed to update startup 0: '.$GLOBALS['mysqli']->error);
    
    // Add steam config items
    $GLOBALS['mysqli']->query("INSERT INTO configuration (config_setting,config_value) VALUES('steam_login_user',''),('steam_login_pass',''),('steam_auth','')") or die('Failed to update configuration: '.$GLOBALS['mysqli']->error);
    
    
    
    // Add SA:MP support
    $GLOBALS['mysqli']->query("INSERT INTO `default_games` (`id`, `cloudid`, `port`, `startup`, `steam`, `gameq_name`, `name`, `intname`, `working_dir`, `pid_file`, `banned_chars`, `steam_name`, `description`, `install_mirrors`, `install_cmd`, `update_cmd`, `simplecmd`) VALUES('', 9, 7777, 0, 0, 'mtasa', 'GTA: San Andreas MP', 'gta_samp', '', '', '', '', 'Grand Theft Auto: San Andreas - Multiplayer', 'http://files.sa-mp.com/samp03asvr_R4.tar.gz', 'tar -zxvf files.sa-mp.com/samp03asvr_R4.tar.gz; mv samp03/* .; rm -fr samp03 samp03asvr_R4.tar.gz', '', './samp03svr')");
...
if(version_compare($cur_version, '3.0.10') == -1)
{
    ...
    $GLOBALS['mysqli']->query("ALTER TABLE `default_games` ADD `type` ENUM('game','voice','other') DEFAULT 'game' NOT NULL AFTER `steam`") or die('Failed to update default games: '.$GLOBALS['mysqli']->error);
    
    // Drop 'type' from `servers` because, it should really be in `default_games` only
    $GLOBALS['mysqli']->query("ALTER TABLE `servers` DROP `type`") or die('Failed to update default games (2): '.$GLOBALS['mysqli']->error);
    
    // Set ventrilo as a voice server
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET `type` = 'voice' WHERE intname = 'vent'") or die('Failed to update ventrilo: '.$GLOBALS['mysqli']->error);
    
    // Fix case on craftbukkit config
    $GLOBALS['mysqli']->query("UPDATE `default_games` SET `config_file` = 'server.properties' WHERE intname = 'mcraft'") or die('Failed to update minecraft: '.$GLOBALS['mysqli']->error);
    ...
    $GLOBALS['mysqli']->query("ALTER TABLE users ADD sso_user BLOB NOT NULL AFTER last_updated,
    ADD sso_pass BLOB NOT NULL AFTER sso_user") or die('Failed to add sso columns: '.$GLOBALS['mysqli']->error);
    
    // Increase password, add `setpass_3010` so we can see if the new pass style was used
    $GLOBALS['mysqli']->query("ALTER TABLE admins MODIFY `password` VARCHAR(255) NOT NULL") or die('Failed to change admins table (1): '.$GLOBALS['mysqli']->error);
    $GLOBALS['mysqli']->query("ALTER TABLE admins ADD `setpass_3010` tinyint(1) unsigned NOT NULL DEFAULT '0' AFTER `deleted`") or die('Failed to change admins table (2): '.$GLOBALS['mysqli']->error);
    
    // Add `loadavg` table
    $GLOBALS['mysqli']->query("CREATE TABLE IF NOT EXISTS `loadavg` (
                    ...
                    PRIMARY KEY (`id`)
                  ) ENGINE=InnoDB DEFAULT CHARSET=utf8") or die('Failed to add loadavg table: '.$GLOBALS['mysqli']->error);
    
    // Add `token` to network tbl
    $GLOBALS['mysqli']->query("ALTER TABLE network ADD `token` VARCHAR(32) NOT NULL AFTER `ip`") or die('Failed to add token to network table: '.$GLOBALS['mysqli']->error);
    
    // Add `size` to templates tbl
    $GLOBALS['mysqli']->query("ALTER TABLE templates ADD `size` VARCHAR(12) NOT NULL AFTER `status`") or die('Failed to add size to templates table: '.$GLOBALS['mysqli']->error);
    
    /*
    // Get original admin user
    $result_origad = $GLOBALS['mysqli']->query("SELECT id,username FROM admins WHERE deleted = '0' ORDER BY id ASC LIMIT 1");
    $row_origad	   = mysqli_fetch_row($result_origad);
    ...
    $GLOBALS['mysqli']->query("UPDATE admins SET password = '$password' WHERE id = '$orig_ad_id'");
